...
export default function RegisterForm({ employees, doors }: {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await fetch('/api/visits/register', {
      method: 'POST',
  ...
  if (success) return (
    <div className="text-center py-4 space-y-2">
      ...
      <p className="text-green-700 font-medium">Besøkende registrert og innsjekket!</p>
      <button onClick={() => setSuccess(false)} className="text-sm text-indigo-600 hover:underline">Registrer ny</button>
    </div>
  )
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input name="visitorPhone" type="tel" className={inputCls} />
        </div>
        <div>
      ...
      <button type="submit" disabled={loading} className="rounded-md bg-indigo-600 px-5 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-60">
        {loading ? 'Registrerer…' : 'Registrer og sjekk inn'}
      </button>
    </form>
  )
}
